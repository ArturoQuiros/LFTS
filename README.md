# Laravel From the Scratch

## 1. The Basics

[1. How a Route Loads a View](./docs/2-Basics/01-routes.md)

[2. Include CSS and JavaScript](./docs/2-Basics/02-js-y-css%20copy.md)

[3. Make a Route and Link to it](./docs/2-Basics/03-custom-routes.md)

[4. Store Blog Posts as HTML Files](./docs/2-Basics/04-custom-routes-html.md)

[5. Route Wildcard Constraints](./docs/2-Basics/05-wildcard.md)

[6. Use Caching for Expensive Operations](./docs/2-Basics/06-cache.md)

[7. Use the Filesystem Class to Read a Directory](./docs/2-Basics/07-filesystem.md)

[8. Find a Composer Package for Post Metadata](./docs/2-Basics/08-metadata%20copy.md)

[9. Collection Sorting and Caching Refresher](./docs/2-Basics/09-cache2.md)

## 2. Blade

[1. Blade: The Absolute Basics](./docs/3-Blade/1-bladebasics.md)

[2. Blade Layouts Two Ways](./docs/3-Blade/2-blade2ways.md)

[3. A Few Tweaks and Consideration](./docs/3-Blade/3-blade-tweaks.md)

## 3. Working with Databases

[1. Environment Files and Database Connections](./docs/4-4-DB/1-env-db.md)

[2. Migrations: The Absolute Basics](./docs/4-DB/2-db-basics.md)

[3. Eloquent and the Active Record Pattern](./docs/4-DB/3-user-tinker.md)

[4. Make a Post Model and Migration](./docs/4-DB/4-postsInDB.md)

[5. Eloquent Updates and HTML Escaping](./docs/4-DB/5-eloquent.md)

[6. 3 Ways to Mitigate Mass Assignment Vulnerabilities](./docs/4-DB/6-Model.md)

[7. Route Model Binding](./docs/4-DB/7-Migraciones.md)

[8. Your First Eloquent Relationship](./docs/4-DB/8-Relaciones.md)

[9. Show All Posts Associated With a Category](./docs/4-DB/9-filtros.md)

[10. Clockwork, and the N+1 Problem](./docs/4-DB/10-clock.md)

[11. Database Seeding Saves Time](./docs/4-DB/11-seeder.md)

[12. Turbo Boost With Factories](./docs/4-DB/12-factories.md)

[13. View All Posts By An Author](./docs/4-DB/13-PostXAutor.md)

[14. Eager Load Relationships on an Existing Model](./docs/4-4-DB/14-last.md)

## 4. Integrate the Design

[1. Convert the HTML and CSS to Blade](./docs/5-Design/1-html-css.md)

[2. Blade Components and CSS Grids](./docs/5-Design/2-blade-components.md)

[3. Convert the Blog Post Page](./docs/5-Design/03-custom-routes.md)

[4. A Small JavaScript Dropdown Detour](./docs/5-Design/4-javascript.md)

[5. How to Extract a Dropdown Blade Component](./docs/5-Design/5-components.md)

[6. Quick Tweaks and Clean-Up](./docs/5-Design/6-fix.md)

## 5. Search

[1. Search (The Messy Way)](./docs/6-Search/1-messy-way.md)

[2. Search (The Cleaner Way)](./docs/6-Search/2-clean-way.md)

## 6. Filtering

[1. Advanced Eloquent Query Constraints](./docs/7-filter/1-query.md)

[2. Extract a Category Dropdown Blade Component](./docs/7-filter/2-new-component.md)

[3. Author Filtering](./docs/7-filter/3-author.md)

[4. Merge Category and Search Queries](./docs/7-filter/4-merge.md)

[5. Fix a Confusing Eloquent Query Bug](./docs/7-filter/5-fix.md)

## 7. Pagination

[1. Laughably Simple Pagination](./docs/8-Pagination/1-pagination.md)

## 8. Forms and Authentication

[1. Build a Register User Page](./docs/9-Forms-Auth/1-controller.md)

[2. Automatic Password Hashing With Mutators](./docs/9-Forms-Auth/2-hash.md)

[3. Failed Validation and Old Input Data](./docs/9-Forms-Auth/3-validaciones.md)

[4. Show a Success Flash Message](./docs/9-Forms-Auth/4-success.md)

[5. Login and Logout](./docs/9-Forms-Auth/5-log-in-out.md)

[6. Build the Log In Page](./docs/9-Forms-Auth/6-login.md)

[7. Laravel Breeze Quick Peek](./docs/9-Forms-Auth/07.md)

## 9. Comments

[1. Write the Markup for a Post Comment](./docs/10-comments/1.md)

[2. Table Consistency and Foreign Key Constraints](./docs/10-comments/2.md)

[3. Make the Comments Section Dynamic](./docs/10-comments/3.md)

[4. Design the Comment Form](./docs/10-comments/4.md)

[5. Activate the Comment Form](./docs/10-comments/5.md)

[6. Some Light Chapter Clean Up](./docs/10-comments/6.md)

## 10. Newsletters and APIs

[1. Mailchimp API Tinkering](./docs/11-Newsletters%20and%20APIs/1.md)

[2. Make the Newsletter Form Work](./docs/11-Newsletters%20and%20APIs/2.md)

[3. Extract a Newsletter Service](./docs/11-Newsletters%20and%20APIs/3.md)

[4. Toy Chests and Contracts](./docs/11-Newsletters%20and%20APIs/4.md)

## 11. Admin Section

[1. Limit Access to Only Admins](./docs/12-Admin/1.md)

[2. Create the Publish Post Form](./docs/12-Admin/2.md)

[3. Validate and Store Post Thumbnails](./docs/12-Admin/3.md)

[4. Extract Form-Specific Blade Components](./docs/12-Admin/4.md)

[5. Extend the Admin Layout](./docs/12-Admin/5.md)

[6. Create a Form to Edit and Delete Posts](./docs/12-Admin/6.md)

[7. Group and Store Validation Logic](./docs/12-Admin/7.md)

[8. All About Authorization](./docs/12-Admin/8.md)

## 12. Conclusion

[1. Goodbye and Next Steps](./docs/13-Bye/1.md)
